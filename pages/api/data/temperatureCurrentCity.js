...
export async function getCityTemperatureData(cityName, cityId) {
  let citiesArray;
  ...
  const resultData = { cityId, name: cityName, temperature, created }
...
export default async function handler(req, res) {
  await runDelayMiddleware(req, res);
  {
    if (req?.query?.cityId) {
      ...
      const resultData = await getCityTemperatureData(cityName, cityId);
